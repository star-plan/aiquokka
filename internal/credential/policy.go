...
const (
	...
	// Calling an official CLI that manages its own credentials is still allowed
	// when the provider declares OfficialCLIRefresh.
...
// DefaultPolicy is the safe default: consume credentials, do not modify them.
const DefaultPolicy = ReadOnly
...
func ParsePolicy(s string) (Policy, error) {
	...
	default:
		return 0, fmt.Errorf("unknown credential policy %q (want readonly, memory, or persist)", s)
	}
}
